...
export default function ObjectivesEditor({
  ...
  const scopeValue = formData.custom_scope_content || '';
  ...
  const handleScopeChange = (html: string) => {
    setFormData({
      ...formData,
      ...manualEditPatch('scope', html),
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-dark-border dark:bg-dark-surface">
        <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-dark-text">Scope Summary</h3>
        <p className="mb-4 text-sm text-gray-600 dark:text-dark-text-muted">
          A pillar-organized summary that renders at the top of the Scope section, above Deliverables.
          Generated by the AI wizard alongside Deliverables.
        </p>
        <TipTapEditor
          value={scopeValue}
          onChange={handleScopeChange}
          placeholder="Write a scope summary here, leave blank for the default template, or use Generate with AI above."
        />
        <p className="mt-1 text-sm text-gray-500 dark:text-dark-text-muted">
          Leave blank to use the default Scope template. Also editable under Content Editing → Scope.
        </p>
      </div>
