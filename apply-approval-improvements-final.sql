-- Apply Approval System Improvements (Final Version)
-- This script should now work after fixing the status values

-- 1. Add constraints to prevent invalid approval states (safely)
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- 2. Add constraint to ensure only one approval per stage per SOW (safely)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'unique_sow_stage'
    ) THEN
        ALTER TABLE sow_approvals 
...
-- 3. Add constraint to ensure valid user roles (safely)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
...
-- 4. Add constraint to ensure valid stage roles (safely)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'check_stage_role'
    ) THEN
...
-- 15. Verify the improvements
SELECT 'Approval improvements applied successfully!' as status;
